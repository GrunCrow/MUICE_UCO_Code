...
#include <sys/time.h>
#include <stdint.h>
#include <xmmintrin.h> // Cabecera para instrucciones SIMD SSE
...
//g++ -o open_mp open_mp.cpp -fopenmp
//.\open_mp.exe

void gemm_op(DT *A, DT *B, DT *C, size_t M, size_t N, size_t K) {
    size_t m, n, k;
    int n_cores = 2; // Número de núcleos a utilizar

    omp_set_num_threads(n_cores);
    #pragma omp parallel for private(m, n, k) 
    for (m = 0; m < M; m++) {
        for (n = 0; n < N ; n+=4) {
            __m128 c_vec = _mm_load_ps(&C[m*N + n]);           
            for (k = 0; k < K; k++) { // Procesa cada elemento de K
                __m128 a_vec = _mm_set1_ps(A[m*K + k]);
                __m128 b_vec = _mm_load_ps(&B[k*N + n]);
                c_vec = _mm_add_ps(c_vec, _mm_mul_ps(a_vec, b_vec));
            }
            _mm_store_ps(&C[m*N + n], c_vec);
        }
        // Manejo de residuos para N que no es múltiplo de 4 
    }
}
